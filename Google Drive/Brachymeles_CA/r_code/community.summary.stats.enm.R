...
plot(seq(from=0.05,to=1.00,by=0.05), results.enm.means$NETS, pch=18, cex=1.75, col="black",  ylim=c(0,100), xlim=c(0.00,1.05), xlab="Grid Size (Decimal Degrees)", ylab="Percent of Equivalent ENMs per Community")
for(c in 1:length(results.enm)){
  boxplot(results.enm[[c]]$NETS, border="black", boxwex=0.075, at=(seq(from=0.05,to=1.00,by=0.05)[c]), add=TRUE, axes=FALSE,col=rgb(0,0,0,0.25))
}
points(seq(from=0.05,to=1.00,by=0.05), results.enm.means$NETS, pch=23, bg=rgb(1,1,1,1), cex=1.75, col="black")


plot(seq(from=0.05,to=1.00,by=0.05)-0.01, results.enm.means.l$NETS, pch=18, col=rgb(0,114/255,178/255,1),  ylim=c(0,100), xlim=c(0.00,1.05), xlab="Grid Size (Decimal Degrees)", ylab="Percent of Equivalent ENMs per Community")
points(seq(from=0.05,to=1.00,by=0.05)+0.01, results.enm.means.m$NETS, pch=18, col=rgb(0,158/255,115/255,1))
for(c in 1:length(results.enm)){
...
points(seq(from=0.05,to=1.00,by=0.05)-0.01, results.enm.means.l$NETS, pch=23, bg=rgb(1,1,1,1), col=rgb(0,114/255,178/255,1), lwd=2, cex=1.75)
points(seq(from=0.05,to=1.00,by=0.05)+0.01, results.enm.means.m$NETS, pch=23, bg=rgb(1,1,1,1), col=rgb(0,158/255,115/255,1), lwd=2, cex=1.75)
