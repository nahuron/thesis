ses.mpd2 <- function (samp, dis, subsetted=sort(colnames(dis)), null.model = c("taxa.labels", "richness", "frequency", "sample.pool", "phylogeny.pool", "independentswap", "trialswap", "other"), abundance.weighted = FALSE, runs = 999, iterations = 1000, comminmax=NULL, alpha=0.05) 
{
    dis <- as.matrix(dis)
...
ses.mntd2 <- function (samp, dis, subsetted=sort(colnames(dis)), null.model = c("taxa.labels", "richness", "frequency", "sample.pool", "phylogeny.pool", "independentswap", "trialswap", "other"), abundance.weighted = FALSE, runs = 999, iterations = 1000, comminmax=NULL, alpha=0.05) 
{
    dis <- as.matrix(dis)
