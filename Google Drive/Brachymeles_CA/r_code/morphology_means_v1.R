...
brach_fldig <- tapply(brach_limbs$Fldig, INDEX=brach_limbs$Species, FUN=function(x) mean(x,na.rm=TRUE))
brach_fldig <- round(brach_fldig, digits=2)
brach_hldig <- tapply(brach_limbs$Hldig, INDEX=brach_limbs$Species, FUN=function(x) mean(x,na.rm=TRUE))
brach_hldig <- round(brach_hldig, digits=2)
brach_limb <- tapply(brach_limbs$Limbstate, INDEX=brach_limbs$Species, FUN=function(x) mean(x,na.rm=TRUE))
#-----------------------------------------------------------------------------------------
