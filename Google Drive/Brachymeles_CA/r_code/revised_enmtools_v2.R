...
env.files <- list.files(path = "/Users/nicholashuron/Dropbox/STUDENT FOLDERS/Huron, Nick/Huron_Nick_Masters/Datasets/Geographic/locality_data", pattern = "\\.asc$", full.names = TRUE)
env <- stack(env.files)
...
sp.files <- list.files(path = "/Users/nicholashuron/Dropbox/STUDENT FOLDERS/Huron, Nick/Huron_Nick_Masters/Datasets/Geographic/coords_ENMS", pattern = ".csv$", full.names = TRUE)
...
sp.files2 <- list.files(path = "/Users/nicholashuron/Dropbox/STUDENT FOLDERS/Huron, Nick/Huron_Nick_Masters/Datasets/Geographic/coords_ENMS", pattern = ".csv$", full.names = FALSE)
...
save(sp.list, file=paste0("/Users/nicholashuron/Desktop/niche_overlap_tests/","sp.list.v5.rda"))
								
load("/Users/nicholashuron/Desktop/niche_overlap_tests/sp.list.v4.rda")
...
#for (b in 11: 11)	{
			...
			save(list=as.character(ident.list[1]), file=paste0("/Volumes/Nicholas_Huron_Backup/NIT/",ident.list[1],".rda"), compress="xz")
	...
	for (e in 1:ncol(hold))		{
		if(names(sp.list[d]) == names(sp.list[e]))	{
											print("Skip due to same species")
												...
												save(list=as.character(bg.asym.list[1]), file=paste0("/Volumes/Time Machine Backups/BAST/",bg.asym.list[1],".rda"), compress="xz")
												rm(bg.asym.hold)
