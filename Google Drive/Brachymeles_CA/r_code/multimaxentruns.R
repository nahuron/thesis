...
#must obtain objects from revised_enmtools text file!
  #specifically the sp.list file and the env layer stack
...
for (a in 1: length(sp.list))	{
  #for (a in 2)	{
	...
	species.predict <-  predict(object=species.maxent@models[[as.numeric(species.model)]], x=env, progress='text', args=c('pictures=TRUE', 'responsecurves=TRUE', 'outputformat=logistic', 'projectionlayers=/Users/nicholashuron/Dropbox/STUDENT FOLDERS/Huron, Nick/Huron_Nick_Masters/Datasets/Geographic/locality_data/', 'doclamp=TRUE', 'plots=TRUE','applythresholdrule=minimum training presence'))
	writeRaster(species.predict, paste0("/Users/nicholashuron/Dropbox/STUDENT FOLDERS/Huron, Nick/Huron_Nick_Masters/Datasets/Geographic/ENMS_Fall_2016/", as.character(sp.files2[a]), "/", as.character(sp.files2[a]), ".asc"), format="ascii", overwrite=TRUE)
	...
	species.predict <- {}											
	    ...
	    species.maxent <-   maxent(x=env, p= sp.list[[a]]$presence.points, a= sp.list[[a]]$background.points, args=args_mx, removeDuplicates=TRUE, path= paste0("/Users/nicholashuron/Dropbox/STUDENT FOLDERS/Huron, Nick/Huron_Nick_Masters/Datasets/Geographic/ENMS_Fall_2016/", as.character(sp.files2[a])))
	    ...
	    species.predict <-  predict(object=species.maxent@models[[as.numeric(species.model)]], x=env, progress='text', args=c('pictures=TRUE', 'responsecurves=TRUE', 'outputformat=logistic', 'projectionlayers=/Users/nicholashuron/Dropbox/STUDENT FOLDERS/Huron, Nick/Huron_Nick_Masters/Datasets/Geographic/locality_data/', 'doclamp=TRUE', 'plots=TRUE','applythresholdrule=minimum training presence'))
			writeRaster(species.predict, paste0("/Users/nicholashuron/Dropbox/STUDENT FOLDERS/Huron, Nick/Huron_Nick_Masters/Datasets/Geographic/ENMS_Fall_2016/", as.character(sp.files2[a]), "/", as.character(sp.files2[a]), ".asc"), format="ascii", overwite=TRUE)
