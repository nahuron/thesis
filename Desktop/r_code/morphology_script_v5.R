...
rem.cols <- c(18,19,22,31,36,39,40,45)
...
brach_fldig <- tapply(round(exp(brach_morph$Fldig)-1), INDEX=brach_morph$Species, FUN=mean)
brach_fldig <- round(brach_fldig, digits=3)
brach_hldig <- tapply(round(exp(brach_morph$Hldig)-1), INDEX=brach_morph$Species, FUN=mean)
brach_hldig <- round(brach_hldig, digits=3)
brach_limb <- tapply(round(exp(brach_morph$Limbstate)-1), INDEX=brach_morph$Species, FUN=mean)
#-----------------------------------------------------------------------------------------
...
pca.all1 <- prcomp(brach_morph[,2:18], scale=T)

#alternative for noTL
pca.all2 <- prcomp(brach_morph[,2:17], scale=T)

#alternative for reduced
pca.all3 <- prcomp(brach_morph[,2:11], scale=T)
...
palette(sp.palette)
plot(PC.values$ALL[,1],PC.values$ALL[,2], pch=16, cex=0.8, col=as.factor(rownames(PC.values$ALL)), xlab="PC 1", ylab= "PC 2", xlim=c(min(PC.values$ALL[,1]*1.25),max(PC.values$ALL[,1]*1.25)), ylim=c(min(PC.values$ALL[,2]*1.25),max(PC.values$ALL[,2]*1.25)), main=expression(Morphometric~PCA~of~Species~of~italic(Brachymeles)))
points(PC.values$MEANS[,1],PC.values$MEANS[,2], pch=18, cex=2, col=as.factor(rownames(PC.values$MEANS)))
pointLabel(PC.values$MEANS[,1],PC.values$MEANS[,2],paste0(rownames(PC.values$MEANS), " ",brach_limb," (", brach_fldig, ",", brach_hldig,")"), allowSmallOverlap=F, col=palette(sp.palette))
