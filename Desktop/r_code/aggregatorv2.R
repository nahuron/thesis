...
for (a in 1:length(mydata.filepathshort)){
  ...
  if (a == 1) {
    ...
    plot(mydata.density, col=mypal_viridis[a],main="Density Curves for Empirical Communities \nAcross a Range of Spatial Scales (M PAIC)", xlab="Phylogenetic Species Variability", ylim=c(0,7), xaxs="i", yaxs="i")
...
#for (a in 1){  
  ...
  if (a == 1) {
    ...
    plot(mydata.density, col=mypal_viridis[a], ylim=c(0,0.9), xlim=c(-6,6), main="Density Curves for Empirical Communities \nAcross a Range of Spatial Scales (M PAIC)", xlab="Standardized Effect Size Mean Phylogenetic Distance", xaxs="i", yaxs="i")
...
for (a in 1:length(mydata.filepathshort)){
  ...
  if (a == 1) {
    ...
    if(length(sig.hold[!is.na(sig.hold)])>=1 & length(nonsig.hold[!is.na(nonsig.hold)])>=1)  {
      ...
      plot(mydata.density, col=mypal_viridis[a], ylim=c(0,1), xlim=c(-6,6), main="Density Curves for Empirical Communities \nAcross a Range of Spatial Scales (M PAIC)", xlab="Standardized Effect Size Mean Nearest Taxon Distance", xaxs="i", yaxs="i")
      ...
      plot(mydata.density, col=mypal_viridis[a], ylim=c(0,1), xlim=c(-6,6), main="Density Curves for Empirical Communities \nAcross a Range of Spatial Scales (M PAIC)", xlab="Standardized Effect Size Mean Nearest Taxon Distance")
...
plot(density(mydata.means$ses.mpd, bw=bw), xlim=c(-1,1), ylim=c(0,3), main="Density Curves for Mean of Empirical Communities \nAcross a Range of Spatial Scales (M PAIC)", xlab="Standardized Effect Size Mean Phylogenetic Distance")
abline(v=0, lty=2)
if(length(sig.hold) > 0)  {
  lines(density(mydata.means$ses.mpd[sig.hold], kernel="triangular", bw=bw), col="blue")
}
if(length(nonsig.hold) > 0)  {
  lines(density(mydata.means$ses.mpd[nonsig.hold], kernel="triangular", bw=bw), col="blue", lty=5)
}

#ses.mntd
#bw <- 0.05424#luzon only
bw <- 0.08518#mindanao only
plot(density(mydata.means$ses.mntd), ylim=c(0,5), xlim=c(-1,1), main="Density Curves for Mean of Empirical Communities \nAcross a Range of Spatial Scales (M PAIC)", xlab="Standardized Effect Size Mean Nearest Taxon Distance")
abline(v=0, lty=2)
if(length(sig.hold) > 0)  {
  lines(density(mydata.means$ses.mntd[sig.hold], kernel="triangular", bw=bw), col="blue")
}
if(length(nonsig.hold) > 0)  {
  lines(density(mydata.means$ses.mntd[nonsig.hold], kernel="triangular", bw=bw), col="blue", lty=5)
}
...
if(length(nonsig.hold) > 0)  {
  lines(density(mydata.means$psv[nonsig.hold], kernel="triangular", bw=bw), col="blue", lty=5)
}
