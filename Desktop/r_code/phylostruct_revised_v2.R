phylostruct.rev <- function (samp, tree, sp=NA, comminmax=NULL, env = NULL, metric = c("psv", "psr", "pse", "psc", "sppregs"), null.model = c("frequency", "richness", "independentswap", "trialswap", "other"), runs = 100, it = 1000, alpha = 0.05, fam = "binomial") 
{
    ...
    if (metric == "sppregs") {
        ...
        return(list(metric = metric, null.model = null.model, runs = runs, it = it, obs = obs, mean.null = mean.null, quantiles.null = quantiles.null, phylo.structure = NULL, nulls = nulls))
    			...
    			if (missing(comminmax))	{
    			  ...
    			  if(any(is.na(sp))==FALSE){Species <- Species <- as.character(sp);print("user specified species")}
    			  else{Species <- unique(tree$tip.label)}
