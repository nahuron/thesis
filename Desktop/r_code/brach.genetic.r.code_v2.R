...
community.files.short <- gsub("\\.csv$","",list.files(path= "/Users/nicholashuron/Dropbox/STUDENT FOLDERS/Huron, Nick/Huron_Nick_Masters/Datasets/Community/communitiesv2", full.names=F))
...
#for (a in 1:1)	{
  ...
  emp.comm <- emp.comm[lapply(emp.comm, class)!="factor"]
	...
	if(length(grep("_L", community.files.short[a]))>=1) {
	  psv.hold <- phylostruct.rev(emp.comm, brach, c(2,5), env=NULL, sp=as.character(brach_fr_key_l$Species), metric="psv", null.model="other", runs=1000, alpha= 0.05)
	}
	else if(length(grep("_M", community.files.short[a]))>=1)  {
